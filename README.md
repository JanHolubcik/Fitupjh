...
- **Client-Side Components & Hooks**: Tests frontend pages, components (`Login`, `Signup`, `BiometricAndGoals`, and `ChangePassword`), and custom hooks (`useTodayMacros` and `useCalculateRecommendedCalories`) in a simulated DOM environment (`happy-dom`) to verify form rendering, loading states, validation error displays, checkbox guards, calorie targets, and correct interaction with the `better-auth` client.
